...
import { SaidaDiretaDto, Origem, ProdutoSpDto, PropsPST, ResponseGetAddress } from './index.interface';
...
@Component({
  selector: 'app-saida-direta-modal',
  standalone: true,
  imports: [CommonModule, FormsModule, BaseModalComponent, MatIconModule, MatTooltipModule, MatSnackBarModule, AddProdutoModal],
  templateUrl: './saida-direta-modal.html',
  styleUrl: './saida-direta-modal.scss'
})
export class SaidaDiretaModal extends ModalBase implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.sub = this.wsService.messages$.subscribe(data => {
      if (!data || !this.isOpen) return;
       ...
       if (data.type === 'saida_direta_resposta') {
        if (data.status === 'ok') {
          this.toastr.success(data.mensagem || 'Saída direta realizada com sucesso!', 'Sucesso', 10000);
  ...
  submitE() {    
    ...
    if (this.wsService.UserCurrent) {
      ...
      const movimentacaoDto: SaidaDiretaDto = {
